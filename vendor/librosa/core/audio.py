...
# Upstream librosa unconditionally imports `soundfile` and `audioread` here
# to implement librosa.load()/stream()/get_duration(filename=...)/
# get_samplerate() (file-path decoding). audio-tools never calls those: our
# own nodes decode caller-supplied WAV/PCM bytes with Python's stdlib `wave`
# module and hand librosa a plain (y, sr) numpy array instead. We drop both
# imports because `soundfile`'s wheel bundles a compiled libsndfile.so
# (LGPL-2.1) and `audioread` is unused, and stub the handful of functions
# below that referenced them so the rest of this module (resample, to_mono,
# zero_crossings, clicks, tone, chirp, autocorrelate, lpc, mu_compress,
# mu_expand — all pure numpy/scipy) keeps working unmodified.
sf = None
audioread = None
# ---------------------------------------------------------------------------
import numpy as np
...
def load(
    ...
    *,
    ...
    # christiangeorgelucas/audio-tools patch: file-path decoding (soundfile/
    # audioread) is intentionally not vendored — see the module-level note
    # above. Nodes decode caller-supplied bytes themselves (nodes/_audio_io.py)
    # and call librosa's array-based functions (stft, feature.*, onset.*,
    # beat.*, resample, to_mono, effects.trim, ...) directly with (y, sr).
    raise NotImplementedError(
        "librosa.load(path) is not available in this vendored, offline build "
        "(no soundfile/audioread backend). Decode audio bytes yourself into "
        "a (y, sr) numpy array/int pair and call librosa's array-based "
        "functions directly."
    )


# christiangeorgelucas/audio-tools patch: __soundfile_load()/__audioread_load()
# (upstream file-decoding helpers behind librosa.load()) are removed — not
# vendored, since we never call load(). See the module-level note above.
...
def stream(
    ...
    *,
    ...
    # christiangeorgelucas/audio-tools patch: file-path streaming (soundfile)
    # is not vendored — see the module-level note above. audio-tools nodes
    # operate on caller-supplied bounded-size clips, not streamed files.
    raise NotImplementedError(
        "librosa.stream(path) is not available in this vendored, offline "
        "build (no soundfile backend)."
    )
...
def get_duration(
    *, y=None, sr=22050, S=None, n_fft=2048, hop_length=512, center=True, filename=None
    ...
    if filename is not None:
        # christiangeorgelucas/audio-tools patch: file-path duration lookup
        # (soundfile/audioread) is not vendored — see the module-level note
        # above. Pass y/sr (or S/sr) instead; our nodes always have the
        # decoded array in hand already.
        raise NotImplementedError(
            "get_duration(filename=...) is not available in this vendored, "
            "offline build (no soundfile/audioread backend); pass y/sr or S/sr."
        )
...
def get_samplerate(path):
    ...
    22050
    """
    # christiangeorgelucas/audio-tools patch: file-path sample-rate lookup
    # (soundfile/audioread) is not vendored — see the module-level note above.
    raise NotImplementedError(
        "get_samplerate(path) is not available in this vendored, offline "
        "build (no soundfile/audioread backend)."
    )
